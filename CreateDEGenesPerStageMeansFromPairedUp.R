...
df_stage_III_data                  <-read.table(file = df_stage_III_file, sep = '\t', header = TRUE,fill=TRUE)                 #
#######################################################################################################################################
...
{	
  ...
  DE_genes        <- list_of_genes[[as.integer(comparisson_index)]]
	...
	write_tsv(log2change_Stage_i, paste(output_dir,"DE_GenesPerStageMeansFromPairedUp_Stage_o",Stage_i,".tsv",sep=""))
	####################################################################################################################	
}
