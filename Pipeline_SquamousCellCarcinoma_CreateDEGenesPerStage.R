...
for (stage_pair in rownames(df_stages))
{
	...
	# Run DESeq2
	dds_stages <- DESeqDataSetFromMatrix(countData = unstranded_data[,colData$patient_id], colData=colData[colnames(unstranded_data),], design = ~  age_at_index +  gender +stages  )
