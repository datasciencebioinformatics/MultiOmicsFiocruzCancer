...
sample_normal   <-colData[colData$tissue_type=="Normal","patient_id"]

#######################################################################################################################################
df_table_comparisson=rbind(data.frame(Stage_i="sample_stage_I",Normal="sample_normal"),
rbind(data.frame(Stage_i="sample_stage_II",Normal="sample_normal"),
rbind(data.frame(Stage_i="sample_stage_III",Normal="sample_normal"))))
####################################################################################################################
...
{	
	...
	Normal_i         <-df_table_comparisson[comparisson_index,"Normal"]
	...
	Normal_i_sample         =list_of_comparisson[[Normal_i]]	
	
	# Take RPKM of genes from samples of each stage
	Stage_i_samples_expr         <-na.omit(unstranded_data[DE_genes,Stage_i_samples])
	Normal_i_sample_sample_expr  <-na.omit(unstranded_data[DE_genes,Normal_i_sample])
	####################################################################################################################
	# Log2foldchange
	LOG_CONSTANT=0.001
	log2change=rowMeans(log(Stage_i_samples_expr+LOG_CONSTANT,2))/rowMeans(log(Normal_i_sample_sample_expr+LOG_CONSTANT,2))
  ...
  ####################################################################################################################		
	...
	# Create volcano plot
	p1 <- ggplot(log2change_Stage_i, aes(log2change, -log(FDR,10))) +  geom_point(size = 2/5) +  theme_bw() + xlim(-2, 2)
