...
stage_specific_genes_stage_I<-stage_specific_genes[stage_specific_genes$SYMBOL %in% stage_I_biomarkers,]
stage_specific_genes_stage_I<-head(stage_specific_genes_stage_I[order(-stage_specific_genes_stage_I$Stage_I.normal.log2fc),"SYMBOL"],n=5)


stage_specific_genes_stage_II<-stage_specific_genes[stage_specific_genes$SYMBOL %in% stage_II_biomarkers,]
stage_specific_genes_stage_II<-head(stage_specific_genes_stage_II[order(-stage_specific_genes_stage_II$Stage_II.normal.log2fc),"SYMBOL"],n=5)

stage_specific_genes_stage_III<-stage_specific_genes[stage_specific_genes$SYMBOL %in% stage_III_biomarkers,]
stage_specific_genes_stage_III<-head(stage_specific_genes_stage_III[order(-stage_specific_genes_stage_III$Stage_III.normal.log2fc),"SYMBOL"],n=5)


p_stage_I_unpaired<-ggplot(unstranded_data_samples_unapaired[unstranded_data_samples_unapaired$SYMBOL %in% stage_I_biomarkers,], aes(x=stages, y=RPKM, fill=stages)) +  geom_boxplot()+ facet_wrap(~SYMBOL, nrow = 1,ncol = 5, scales="free")+ theme_bw() + ggtitle("A)") + scale_fill_manual(values=c('#e8f2a1', '#729fcf', '#ffaa95')) + stat_summary(fun.y=mean, fun.min = min, fun.max = max ,geom="point", shape=20, size=8, color="red", fill="red")+ geom_jitter(aes(colour = stages), size=0.4, alpha=0.9) + scale_color_manual(values=c('#e8f2a1', '#729fcf', '#ffaa95')) + stat_compare_means(comparisons = my_comparisons, method = "t.test", vjust = 1.0, symnum.args=symnum.args) + theme(axis.text.x = element_text(angle = 90), axis.title.x=element_blank()) 
p_stage_II_unpaired<-ggplot(unstranded_data_samples_unapaired[unstranded_data_samples_unapaired$SYMBOL %in% stage_specific_genes_stage_II,], aes(x=stages, y=RPKM, fill=stages)) +  geom_boxplot()+ facet_wrap(~SYMBOL, nrow = 5,ncol = 5, scales="free")+ theme_bw() + ggtitle("B)") + scale_fill_manual(values=c('#e8f2a1', '#729fcf', '#ffaa95')) + stat_summary(fun.y=mean, fun.min = min, fun.max = max ,geom="point", shape=20, size=8, color="red", fill="red")+ geom_jitter(aes(colour = stages), size=0.4, alpha=0.9) + scale_color_manual(values=c('#e8f2a1', '#729fcf', '#ffaa95')) + stat_compare_means(comparisons = my_comparisons, method = "t.test", vjust = 1.0, symnum.args=symnum.args)	+ theme(axis.text.x = element_text(angle = 90), axis.title.x=element_blank()) 
p_stage_III_unpaired<-ggplot(unstranded_data_samples_unapaired[unstranded_data_samples_unapaired$SYMBOL %in% stage_specific_genes_stage_III,], aes(x=stages, y=RPKM, fill=stages)) +  geom_boxplot()+ facet_wrap(~SYMBOL, nrow = 5,ncol = 5, scales="free")+ theme_bw() + ggtitle("C)") + scale_fill_manual(values=c('#e8f2a1', '#729fcf', '#ffaa95')) + stat_summary(fun.y=mean, fun.min = min, fun.max = max ,geom="point", shape=20, size=8, color="red", fill="red")+ geom_jitter(aes(colour = stages), size=0.4, alpha=0.9) + scale_color_manual(values=c('#e8f2a1', '#729fcf', '#ffaa95')) + stat_compare_means(comparisons = my_comparisons, method = "t.test", vjust = 1.0, symnum.args=symnum.args)	+ theme(axis.text.x = element_text(angle = 90)) 


png(filename=paste(output_folder,paste("p_stages_all_unpaired_selected.png",sep=""),sep=""), width = 24, height = 30, res=600, units = "cm")
	ggarrange(p_stage_I_unpaired, p_stage_II_unpaired, p_stage_III_unpaired, nrow=3, common.legend = TRUE, legend="bottom")
dev.off()
